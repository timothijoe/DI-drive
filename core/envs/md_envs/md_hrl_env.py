...
from core.utils.simulator_utils.md_utils.agent_manager_utils import HrlAgentManager
...
DIDRIVE_DEFAULT_CONFIG = dict(
    ...
    # If not, we will use jerk control, the current state we have vel, acc, current steer, and the control signal is jerk and 
...
class MetadriveHrlEnv(BaseEnv):
    ...
    def __init__(self, config: dict = None):
        ...
        self.agent_manager = HrlAgentManager(
    ...
    def _step_macro_simulator(self, actions):
        ...
        for frame in range(frames):
            ...
            scene_manager_before_step_infos = self.engine.before_step_hrl(frame, wps)
            self.engine.step()
...
register(
    id='HRL-v1',
    entry_point='core.envs.md_envs.md_hrl_env:MetadriveHrlEnv',
)
