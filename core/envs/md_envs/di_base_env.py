...
from core.utils.simulator_utils.md_utils.agent_manager_utils import DiAgentManager
...
DIDRIVE_DEFAULT_CONFIG = dict(
    ...
    # ===== Trajectory length =====
    # mainly used in hrl env, for other envs, just ignore the parameter
    seq_traj_len = 1,
    show_seq_traj = False,
    ...
    # If traj_control_mode = 'jerk' , we will use jerk control, the current state we have vel, acc, current steer, and the control signal is jerk and 
    # steer rate (delta_steer)
    # If traj_control_model = 'simple', only observation will sent rather than a dict containing vehicle satate
    traj_control_mode = 'simple',
)


class DiBaseEnv(BaseEnv):
    ...
    def __init__(self, config: dict = None):
        ...
        self.agent_manager = DiAgentManager(
    ...
    def _step_simulator(self, actions):
        ...
        # step all entities
        self.engine.step(self.config['repeat_times'])
    ...
    def _post_process_config(self, config):
        config = super(DiBaseEnv, self)._post_process_config(config)
    ...
    def reward_function(self, vehicle_id: str):
        ...
        if not self.config['use_sparse_reward']:
            ...
            if self.config["use_jerk_reward"]:
                ...
                #print('jerk_penalty: {}'.format(jerk_penalty))
    ...
    def setup_engine(self):
        super(DiBaseEnv, self).setup_engine()
...
register(
    id='DiBase-v0',
    entry_point='core.envs.md_envs.di_base_env:DiBaseEnv',
)
