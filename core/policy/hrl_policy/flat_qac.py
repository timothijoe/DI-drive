...
# from core.policy.hrl_policy.variate_vae_decoder import VaeDecoder
from core.policy.hrl_policy.flat_vae import VaeDecoder
...
class FlatConvQAC(QAC):
    ...
    def __init__(
        ...
        self._traj_decoder = VaeDecoder(
            ...
            seq_len = vae_seq_len,
            dt = vae_dt,
            steer_rate_constrain_value=steer_rate_constrain_value,
        )
    ...
    def compute_actor(self, inputs: torch.Tensor) -> Dict:
        obs_birdview = inputs['birdview']
        if self.action_space == 'regression':
            x = self.actor(obs_birdview)
            return {'action': x['pred']}
        elif self.action_space == 'reparameterization':
            x = self.actor(obs_birdview)
            return {'logit': [x['mu'], x['sigma']]}
        elif self.action_space == 'hybrid':
            logit = self.actor[0](obs_birdview)
            action_args = self.actor[1](obs_birdview)
            return {'logit': logit['logit'], 'action_args': action_args['pred']}

    def compute_critic(self, inputs: Dict) -> Dict:
        obs, latent_action = inputs['obs'], inputs['latent_action']
        obs_birdview = obs['birdview']
        if self.twin_critic:
            x = [m(obs_birdview) for m in self.critic_encoder]
            x = [torch.cat([x1, latent_action], dim=1) for x1 in x]
            x = [m(xi)['pred'] for m, xi in [(self.critic_head[0], x[0]), (self.critic_head[1], x[1])]]
        else:
            x = self.critic_encoder(obs_birdview)
            torch.cat([x, latent_action], dim=1)
            x = self.critic_head(x)['pred']
        return {'q_value': x}
