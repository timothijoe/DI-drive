...
class VaeEncoder(nn.Module):
    ...
    def encode(self, input):
        ...
        traj_embedding = self.spatial_embedding(data_traj.view(-1, 6))
...
class TrajVAE(nn.Module):
    ...
    def loss_function(self, *args):
        ...
        theta_error = F.mse_loss(recons[:,:,2], input[:,:,2] * np.pi / 180) * 0.01
        final_theta_error = F.mse_loss(recons[:,-1,2], input[:,-1,2] * np.pi / 180) * 10
        kld_loss = torch.mean(-0.5 * torch.sum(1 + log_var - mu ** 2 - log_var.exp(), dim=1), dim=0)
        kld_weight = 0.01
        loss = recons_loss  + kld_weight * kld_loss + self.fde_weight * final_displacement_error + theta_error  + vel_loss + final_theta_error
...
#     def plant_model_batch1(self, prev_state_batch, pedal_batch, steering_batch, dt = 0.03):
#         #import copy
#         prev_state = prev_state_batch
#         x_t = prev_state[:,0]
#         y_t = prev_state[:,1]
